...
public class ID_Validator
{
    ...
    public static bool Validate_ID(long id_num)
    {
        ...
        // Check to see if ID starts with a 0 -> invalid
        if (id[0] == '0') 
            return false;

        // Check to see if ID is odd length -> valid
        if (id.Length % 2 == 1) 
            return true;

        // Split ID into two halves and check if they are equal
        string firstHalf = id[..(id.Length / 2)];
        string secondHalf = id[(id.Length / 2)..];
